...
public class ReceiptImageController {
    ...
    public ReceiptSuggestionResponse parseReceipt(@NotEmpty String base64EncodedImage) throws Exception {
        ...
        try (ImageAnnotatorClient client = ImageAnnotatorClient.create()) {
            ...
            //List<com.google.cloud.vision.v1.BoundingPoly> posList = new ArrayList<>();
            boolean once = false;
            for (EntityAnnotation annotation : res.getTextAnnotationsList()) {
                //out.printf("Position : %s\n", annotation.getBoundingPoly());
                //out.printf("Text: %s\n", annotation.getDescription());
                if (!once) {
                    merchantName = annotation.getDescription().split("\n")[0];
                    once = true;
                }
                try {
                    String curVal = annotation.getDescription();
                    if (curVal.charAt(0) == '$') {
                        curVal = curVal.substring(1,curVal.length());
                    }
                    if (curVal.charAt(curVal.length() - 3) != '.') {
                        throw new Exception("Not proper format");
                    }
                    amount = new BigDecimal(curVal);
                    out.printf("Did add: %s\n", curVal);
                } catch (Exception e) {
                    out.printf("Did not add: %s\n", annotation.getDescription());
                }
            }
